...
export const zhCN: Translations = {
  ...
  command: {
    ...
    extract: {
      ...
      file: {
        ...
        visionTranscribed: '已通过 AI 视觉模型转写图片文本（{{model}}）',
        visionTranscribeFailed: '视觉模型 {{model}} 转写失败，降级为本地 OCR',
        ...
        extractRetryChunk: '切片 {{current}}/{{total}} API 返回 {{code}}，{{delay}} 秒后重试（{{attempt}}/{{max}}）',
        extractFailChunk: '切片 {{current}}/{{total}} 提取失败',
        validationFail: '合并数据校验失败',
  ...
  webUi: {
    ...
    imageInput: '图片输入',
    imageInputSummary: {
      visionModel: '图片会通过 AI 视觉模型转写后进入结构化抽取。',
      ocrNoModel: '未配置视觉模型，降级为本地 OCR。',
      ocrLocal: '图片文本将通过本地 OCR 读取。',
    },
    ...
    ocrHint: '图片会通过 AI 视觉模型或本地 OCR 转换为文本，再进入结构化抽取。',
